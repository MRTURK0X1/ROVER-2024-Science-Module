...
uint8_t readDHT11(dht11_t *dht)
{
	...
	uint8_t humValI = 0, tempValI = 0, humValD = 0, tempValD = 0, parityVal = 0, genParity = 0;
	uint8_t mData[40];
...
//	HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_SET);
	...
	//get integral hum value from data buffer
	for(int i = 0; i < 8; i++)
	{
		humValI += mData[i];
		humValI = humValI << 1;
	...
	//get decimal hum value from data buffer
	for(int i = 8; i < 16; i++)
	{
		humValD += mData[i];
		humValD = humValD << 1;
	...
	//get integral temp value from data buffer
	for(int i = 16; i < 24; i++)
	{
		tempValI += mData[i];
		tempValI = tempValI << 1;
	...
	//get decimal temp value from data buffer
	for(int i = 24; i < 32; i++)
	{
		tempValD += mData[i];
		tempValD = tempValD << 1;
	...
	genParity = humValI + tempValI;

	//if(genParity == parityVal)

	dht->temperature_I = tempValI;
	dht->humidty_I = humValI;

	dht->temperature_D = tempValD;
	dht->humidty_D = humValD;

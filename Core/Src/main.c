...
int main(void)
{
  ...
  //SENSOR INITS
  init_dht11(&DHT11,&htim8,GPIOC,GPIO_PIN_8);
  BMP180_Init(&hi2c1);
  MQ_Init(&mq137,&hadc1,temp,hum);
  MQ_Init(&mq4,&hadc4,temp,hum);
  MQ_Init(&mq135,&hadc3,temp,hum);

  //PWM INITS
  //CCR Registerine verilen değerler duty cycle yüzdesini ifade eder.
  //SAĞDAN 1. VICTOR
